...
cleanup() {
  rm -rf "$tmp"
  rm -f "$review" "$decline_review" "$artifact" "$rerun_artifact" "$decline_artifact"
}
trap cleanup EXIT

mkdir -p "$review_dir" "$triage_dir" "$tmp/bin"
for test_path in "$review" "$decline_review" "$artifact" "$rerun_artifact" "$decline_artifact"; do
  if [[ -e "$test_path" ]]; then
...
grep -Fq "TITLE: Add boundary-condition coverage" "$tmp/gh.log"
